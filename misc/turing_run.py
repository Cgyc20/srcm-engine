...
with PDE-style cosine perturbation initial conditions (steady-state + bump).
...
def turing_pde_terms(C: np.ndarray, rates: dict) -> np.ndarray:
    ...
    If your PDE operator inside SRCMEngine expects concentration instead,
    you may need to convert by dividing by h internally.
    """
    U, V = C[0], C[1]
...
def build_engine():
    # ----------------------------
    # Controls to match your non-hybrid script
    # ----------------------------
    N_COMPARTMENTS = 12
    OMEGA = 5000
    PERT_A = 0.07
    ...
    total_time = 100.0
    dt = 0.001  # match your SSA/PDE script’s timestep (you can switch back to 0.01 if you want)

    # ----------------------------
    # Rates (your "bar" params)
    # ----------------------------
    bar_r_11 = 1.0
    bar_r_12 = 1.0
    bar_r_2 = 2.0
    ...
    reactions.add_hybrid_reaction(
        ...
        state_change={"D_U": +1},
        label="R11_DC",
        description="D^U + C^U -> 2D^U + C^U (factor 2)",
    )
    ...
    reactions.add_hybrid_reaction(
        ...
        propensity=lambda D, C, r, h: r["r_12"] *D["U"] *(D["U"]-1) / h,
        state_change={"D_V": +1},
        label="R21_CC",
        description="2C^U -> 2D^U+D^V",
    )

    reactions.add_hybrid_reaction(
        reactants={"D_U": 1, "C_U":1},
        products={"D_U": 1, "C_U": 1, "D_V":1},
        propensity=lambda D, C, r, h: 2*r["r_12"] * D["U"] * C["U"] / h,
        state_change={"D_V": +1},
        label="R12_DD",
        description="D^U + C^U-> D^U + C^U + D^V (factor 2)",
    )
    ...
    reactions.add_hybrid_reaction(
        ...
        products={"D_V": 1},
        propensity=lambda D, C, r, h:  r["r_2"] * D["U"] * D["V"] / h,
        state_change={"D_U": -1},
        label="R31_DC",
        description="D^U + D^V -> D^V",
    )
    reactions.add_hybrid_reaction(
        reactants={"D_U": 1, "C_V":1},
        products={"C_V": 1},
        propensity=lambda D, C, r, h: r["r_2"] * D["U"]*C["V"] / h,
        state_change={"D_U": -1},
        label="R32_CC",
        description="D^U + C^V-> C^V",
    )

    # --- Reaction C: U + V -> V (net -1 U), rate r_2 ---
    reactions.add_hybrid_reaction(
        reactants={"C_U": 1, "D_V": 1},
        products={"D_V": 1},
        propensity=lambda D, C, r, h: r["r_2"] * C["U"] * D["V"] / h,
        state_change={"C_U": -1},
        label="R33_DD",
        description="C^U + D^V -> D^V",
    )
    

    # --- Reaction D: V -> 0 (net -1 V), rate r_3 ---
    reactions.add_hybrid_reaction(
        reactants={"D_V": 1},
        products={},
...
def initial_conditions(domain: Domain, meta: dict):
    """
    Cosine-perturbed steady state, matching your non-hybrid SSA script.

    Returns:
      init_ssa: (2, K) ints
      init_pde: (2, n_pde) floats
    """
    ...
    # bar rates for steady state (dimensionless steady state)
